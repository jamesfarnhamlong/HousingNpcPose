...
public sealed class ConfigWindow : Window, IDisposable
{
    ...
    {
        ImGui.TextWrapped("v0.4.1. Saved local pose assignments, scene presets, and optional visual Y offsets can be auto-applied or loaded after entering/reloading an area. Nameplate hiding can remove labels from posed/saved NPCs. Everything is client-side only.");
        ...
        ImGui.Separator();
        ImGui.TextUnformatted($"Saved pose entries: {configuration.SavedPoses.Count} | Scene presets: {configuration.ScenePresets?.Count ?? 0} | Discovery observations: {configuration.PoseObservations?.Count ?? 0}");
